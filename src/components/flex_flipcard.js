...
`;
let y = 1;
class FlexFlipCard extends Component {
  constructor(props) {
    super(props);
    this.state = {
      isCardFlipped: false
  ...
  findAndSetCardFlipper(childComponent, flipperRefName) {
    ...
    if (childComponent.ref === flipperRefName) {
      ...
      React.Children.count(childComponent) > 0 &&
      ...
      return React.cloneElement(childComponent, {
        children: React.Children.map(
          ...
          grandchild => this.findAndSetCardFlipper(grandchild, flipperRefName)
        )
      });
    }
  ...
  render() {
    ...
    const children = React.Children.map(this.props.children, child =>
      this.findAndSetCardFlipper(child, this.props.flipperID || 'flipper')
    );
